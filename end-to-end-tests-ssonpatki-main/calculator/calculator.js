/**
 * Main calculator module with core functionality
 */
 ...
 * numeral.  Old Roman numerals are based only on addition.
 *
 ...
 */
export function convertToOldRoman(input) {
 ...
 *
 * @returns {Promise<string>} Returns a promise that resolves to a string containing
 *   the modern Roman numeral conversion for the specified input value.
 */
export async function convertToModernRoman(input) {
  if (input === "0") {
    return "0";
  }
...
/*
 * Updates which links are displayed in the navbar based on
...
export function updateNavbar() {
  ...
  const registerNavItem = document.getElementById("nav-register");
  const unregisterNavItem = document.getElementById("nav-unregister");

  const isLoggedIn = window.localStorage.getItem("token");
  if (isLoggedIn) {
    registerNavItem.classList.add("hidden");
    loginNavItem.classList.add("hidden");
    unregisterNavItem.classList.remove("hidden");
    historyNavItem.classList.remove("hidden");
    logoutNavItem.classList.remove("hidden");
  } else {
    unregisterNavItem.classList.add("hidden");
...
/**
 * Main calculator function that initializes the calculator UI and functionality
 */
